...
void jointCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
 ...
 /* if(!(count%50)){
  ...
  double backstart = -PI/4;
  double backend = -0;
  double backstep = (backend-backstart)/2; 

  double frontstart = 0;
  double frontend = PI/4;
  ...
  lb_k_msg.data = (backstart + (backstep)*count/countmax)*(phase==0) + (backstart+backstep + backstep*count/countmax)*(phase==1) +
  ...
  rb_k_msg.data = (backstart + (backstep)*count/countmax)*(phase==2) + (backstart+backstep + backstep*count/countmax)*(phase==3) +
